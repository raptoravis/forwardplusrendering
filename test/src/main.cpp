...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow )
{
	...
	for (int i = 0; i < numArgs; i++)
	{
		...
		else if (wcscmp(commandLineArguments[i], L"-p") == 0 || wcscmp(commandLineArguments[i], L"--path") == 0)
		{
			bSetCurrentExePath = true;
		}
	}

	if (bSetCurrentExePath)
	{
